...
import {
  getApplications, applyWorkflowAction,
  type Application, type WorkflowStep,
} from "@/lib/applicationStore";
import { getRegistrations } from "@/lib/registrationStore";
import {
  COMMITTEE_MEMBERS_POOL, createMeeting, simulateEmail,
  getNextMeetingNumber, getMeetingsByType,
  type MeetingType, type CommitteeMember, type CommitteeMeeting as MeetingRecord,
...
const CommitteeMeeting = () => {
  ...
  const requiredMembers = isICVD ? 2 : 3;
  ...
  useEffect(() => {
    ...
    setScheduledMeetings(getMeetingsByType(meetingType));
  }, []);
  ...
  const meetingNumber = getNextMeetingNumber(meetingType);
  ...
  const handleSchedule = () => {
    ...
    if (selectedApps.length === 0) {
      toast({ title: "Applications Required", description: "Select at least one application.", variant: "destructive" });
      return;
    }

    const membersSelected: CommitteeMember[] = COMMITTEE_MEMBERS_POOL;
    ...
    toast({
      ...
      description: `${isICVD ? "IC-VD" : "CCIC-CGM"} Meeting #${meetingNumber} scheduled with ${membersSelected.length} members and ${selectedApps.length} applications.`,
    });

    // Refresh meetings list and reset form
    setScheduledMeetings(getMeetingsByType(meetingType));
    setSelectedApps([]);
    setSubject("");
    setDate(undefined);
  ...
  return (
    <AppLayout
      title={`SIDBI — ${title}`}
      subtitle="Committee Meeting Scheduling"
      ...
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        <div className="bg-card border border-border">
          ...
          <div className="p-6 space-y-4">
            <div className="grid grid-cols-[160px_1fr] gap-4 items-start">
              ...
              <Textarea
                placeholder={`${isICVD ? "IC-VD" : "CCIC-CGM"} Meeting Review #${meetingNumber}`}
                value={subject}
        ...
        <div className="bg-card border border-border">
          <div className="gov-section-header bg-muted px-6 py-3 border-b border-border flex items-center justify-between">
            <h2 className="font-bold text-foreground text-sm uppercase tracking-widest">
              {`Applications (${isICVD ? "Once that are approved by Checker to IC-VD" : "Referred from IC-VD"})`}
            </h2>
          ...
          <div className="p-6">
            {eligibleApps.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-8">
                No applications awaiting {isICVD ? "IC-VD" : "CCIC-CGM"} committee scheduling.
              </p>
            ) : (
                   ...
                   {eligibleApps.map((app, idx) => (
                     <tr
                       ...
                       className={cn(
                         ...
                         selectedApps.includes(app.id) && "bg-primary/5"
                       )}
                     ...
                     >
                       ...
                       <td className="px-4 py-3">
                         <span
                           ...
                           onClick={(e) => {
                             e.stopPropagation();
                             navigate(`/sidbi/review/${app.id}?back=/sidbi/meeting/${type}`);
                           }}
                         >
        ...
        <div className="flex justify-end">
          <Button
            ...
            className="font-bold uppercase tracking-wider text-xs px-8 py-3"
